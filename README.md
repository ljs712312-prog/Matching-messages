...
- `문자 전체 정리`: 기존 문자 양식을 그대로 붙여 넣어 전화번호와 비고까지 정리합니다.
- `사건번호 빠른 조회`: `24-14090 2번 상가`처럼 사건번호 목록만 넣어 바로 조회합니다. 물건번호를 생략하면 1번으로 처리합니다.
- 내려받는 엑셀에는 전체 주소 열이 있는 `최종결과`와 전달받은 표 형태에 가까운 `사진형목록` 시트가 함께 들어갑니다.

같은 사건의 여러 물건은 한 번만 법원 사이트를 조회하며, 확인된 다른 물건 주소도 캐시에 저장합니다. 서로 다른 사건은 최대 2개씩 동시에 조회해 여러 사건을 넣었을 때의 전체 대기시간을 줄입니다.
